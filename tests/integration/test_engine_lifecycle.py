#!/usr/bin/env python3
"""
integration/test_engine_lifecycle.py — 引擎生命周期集成测试

测试对象: inferfabric.db.IFFDB, state.StateDB, telemetry.TelemetryHub,
         engine_adapter.*, model_lifecycle.ModelLifecycle
覆盖范围:
  - IFFDB: 表创建、PR/active/sleep/history/manual/log CRUD、prune、并发
  - StateDB: get/set、multi、sleep、并发
  - GPUMode 状态转换
  - TelemetryHub: 记录持久化、batch、metrics、token_stats
  - 引擎适配器注册表（全部 7 种）
  - VLLMAdapter: validate、sleep/wake 委托、ctx_window、health、metrics_flags
  - ModelLifecycle: sleep/wake 生命周期、already_sleeping、mutex、unknown_model
"""
