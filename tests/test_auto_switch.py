"""Auto-switch / auto-start logic tests.

Covers the 2026-09-09 incident:
  - Auto-switch to qwen38-27b-abliterated ended up with gemma4-31b-vl
    running instead, because a stray request for gemma4 triggered the
    auto-switch first, and the qwen38 deploy was then rejected by the VRAM
    budget guard.
  - Proxy crash-looped on EADDRINUSE (port 8999 held by a stale process);
    systemd Restart=always restarted it every ~5s.

Fixes verified here:
  1. AUTO_SWITCH defaults to OFF (EDGE_AUTO_SWITCH unset → False); explicit
     "1" still enables it.
  2. _create_server() retries on EADDRINUSE with bounded backoff.
  3. Occupancy guard in manager.switch() cross-checks actual port owners
     (fuser) so a stale "idle" DB state cannot hide a running exclusive
     service (e.g. gemma4 holding port 8005).
"""
