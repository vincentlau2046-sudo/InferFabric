...
class TestNormalizeServerTools:
    """服务器端 Anthropic 工具在 OpenAI 端点上被丢弃。"""

    def test_server_tool_dropped(self):
        """Server-side web_search tool without input_schema → dropped."""
        data = {"tools": [{"type": "web_search_20250305", "name": "web_search", "max_uses": 8}]}
        _normalize_tools_for_openai(data)
        assert data["tools"] == []

    def test_mixed_server_and_anthropic(self):
        """Server tool dropped; Anthropic tool converted."""
        data = {
            "tools": [
    ...
    def test_mixed_server_and_openai(self):
        """Server tool dropped; OpenAI tool preserved."""
        data = {
            "tools": [
                {"type": "web_search_20250305", "name": "web_search", "max_uses": 8},
    ...
    def test_computer_tool_dropped(self):
        """server-side computer_20250124 tool → dropped."""
        data = {"tools": [{"type": "computer_20250124", "name": "computer"}]}
        _normalize_tools_for_openai(data)
        assert data["tools"] == []
