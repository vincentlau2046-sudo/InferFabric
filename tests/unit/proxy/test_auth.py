"""
unit/proxy/test_auth.py — 鉴权模块测试

测试对象: inferfabric.proxy.auth.AuthManager
覆盖范围:
  - Primary key 校验（Bearer 前缀、空 token、全模型访问）
  - Guest key（模型白名单、过期时间、无过期）
  - key_name() 日志记录
  - 热加载 reload()（fail-closed 策略）
"""
...
class TestAuthReload:
    ...
    def test_reload_keeps_keys_on_invalid_config(self, tmp_path):
        """reload 对无效配置保持旧 key（fail-closed 策略）。"""
        p = tmp_path / "api_keys.yaml"
        _write_yaml(p, {"primary": "sk-old"})
        mgr = AuthManager(p)
        assert mgr.enabled

        p.write_text("")  # clear — reload 会失败但保留旧 key
        mgr.reload(p)
        # fail-closed: 旧 key 保留，不会清空
        assert mgr.enabled
        ok, _ = mgr.check("sk-old", "any")
        assert ok
