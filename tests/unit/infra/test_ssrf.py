"""
unit/integration/test_ssrf.py — SSRF 防护测试

测试对象: inferfabric.proxy.handler.ProxyHandler._validate_cloud_test_url
覆盖范围:
  - 私有 IP 拦截: 127.x, 10.x, 172.16.x, 192.168.x, 169.254.x
  - IPv6 回环 & 私有地址拦截
  - 非 HTTPS / FTP 协议拦截
  - 非白名单域名拦截
  - DNS rebinding 防护（白名单域名解析到私有 IP）
  - DNS 解析失败拦截
  - 合法 URL 放行
  - _handle_cloud_test 集成测试
"""
