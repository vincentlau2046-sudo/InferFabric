"""
unit/infra/test_req_id.py — 请求 ID 线程安全测试

测试对象: inferfabric.proxy_manager.ProxyManager.new_request_id
覆盖范围:
  - req_id 格式: {8-hex-counter}-{8-hex-uuid}
  - 计数器单调递增
  - 100 线程并发生成 10000 个 ID 零碰撞
  - 长度固定 17 字符、包含连字符
"""
