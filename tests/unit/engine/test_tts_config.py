"""
unit/engine/test_tts_config.py — TTS 服务配置与解析测试

测试对象: inferfabric.config.TTSConfig, ModelConfig (type=tts_server)
覆盖范围:
  - TTSConfig 默认值与完整配置
  - resolved_working_dir 解析
  - load_models 对 tts_server 类型的解析（extra_env 保护、顶级 fallback）
  - ModelConfig.is_tts_server / tts_port / tts_served_name / tts_modality
  - 启动命令构建、extra_env 注入
"""
